...
struct sensor sensorArray[sensorNum] = {{1, 6, 5}};       // Enter the sensor type, input pin and output pin
...
void loop() {
  ...
  if(secondNow%3 == 0){
    ...
    // prints the sensor reading to the data file
    for(int i = 0; i < sensorNum ; i++){
      switch (sensorArray[i].sensorType) {
        case 0:
          // return some kind of error which should be replaced   
...
struct waterLevel calcLevel(const int trigPin, const int echoPin) {
  ...
  tankLevel.volume = (tankHeight - tankLevel.distance) * 3.14 * sq(tankDiameter / 2) * 1000;  // V = height * pi * r^2 * litre conversion
