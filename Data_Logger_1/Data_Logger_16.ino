...
float bTemp = 24.0;
float bHealth = 80.5;
float dataWater[6] = {wLevel, wFlow, wTurb, wPH, wTemp, wEC};
float dataBattery[2] = {bTemp, bHealth};
int delayPrint = 5000;
  ...
  for(int i = 0; i < sensorNum ; i++){
    switch (sensorArray[i].sensorType) {
      ...
      case 1: // sets up input and output pins for ultra sonic sensor
        ...
        printToFile(waterStatusFile, "Level");
        printToFile(waterStatusFile, ",");
        break;
...
//  if((now.second() % 60) && (now.minute() % 30) == 0){
//    sendBattery = true;
//  }
